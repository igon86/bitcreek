        ...
        while (true) {
            try {
                ...
                try {
                    ...
                    System.out.println(Thread.currentThread().getName()+" INVIO BITFIELD\n");
                    /*
                    ...
                    System.out.println(Thread.currentThread().getName()+" ASCOLTO: AGGIUNGO CONNESSIONE NEL MONITOR");
                    ...
                    if (contacted.getStato() && conn.DownNull() && peer.getConnessioni() < BitCreekPeer.MAXCONNESSIONI) {
                        ...
                        System.out.println(Thread.currentThread().getName()+" HO FATTO LA CONNECT");
                        ObjectOutputStream output = new ObjectOutputStream(mysock.getOutputStream());
                        System.out.println(Thread.currentThread().getName()+" HO FATTO L'OUTPUT");
                        ObjectInputStream input = new ObjectInputStream(mysock.getInputStream());
                        System.out.println(Thread.currentThread().getName()+" HO FATTO L'INPUT");
                        output.writeObject(mycon);
                        Bitfield b = (Bitfield) input.readObject();
                        System.out.println(Thread.currentThread().getName()+" BITFIELD RICEVUTO!!!");
                        // modifica
                        ...
                        System.out.println(Thread.currentThread().getName()+" ASCOLTO CALLBACK: AGGIUNGO CONNESSIONE");
                        peer.incrConnessioni();
                        // incremento numero peer in download
                        contacted.incrPeer();
                    }
                } catch (IOException ex) {
                    System.out.println(Thread.currentThread().getName()+" IOException in Ascolto");
                    Logger.getLogger(Ascolto.class.getName()).log(Level.SEVERE, null, ex);
